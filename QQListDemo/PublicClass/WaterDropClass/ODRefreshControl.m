...
- (id)initInScrollView:(UIScrollView *)scrollView activityIndicatorView:(UIView *)activity {
    self = [super initWithFrame:CGRectMake(0, -(kTotalViewHeight + scrollView.contentInset.top), scrollView.bounds.size.width, kTotalViewHeight)];
    ...
    if (self) {
       ...
       [scrollView addObserver:self forKeyPath:@"contentInset" options:NSKeyValueObservingOptionNew context:nil];
        ...
        _activity.center = CGPointMake(floor(self.bounds.size.width / 2), floor(self.bounds.size.height / 2));
        ...
        _arrowLayer.strokeColor = [[[UIColor lightGrayColor] colorWithAlphaComponent:0.5] CGColor];
        _arrowLayer.lineWidth = 0.5;
        _arrowLayer.fillColor = [[UIColor orangeColor] CGColor];
        [_shapeLayer addSublayer:_arrowLayer];
        
        _highlightLayer = [CAShapeLayer layer];
        _highlightLayer.fillColor = [[[UIColor lightGrayColor] colorWithAlphaComponent:0.8] CGColor];
